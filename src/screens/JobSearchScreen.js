...
function JobSearchScreen() {
  ...
  return (
    <div>
      <Header />
      <div
        className="LargeSearchContainer"
      ...
      <div
        className={
          isMobile ? "JobsSmall" : isTablet ? "JobsMedium" : "JobsLarge"
        }
      >
        {jobsByJobName.map((jobsByJobName) => (
          <JobCard
        ...
        {jobsByCompanyName.map((jobsByCompanyName) => (
          <JobCard
        ...
        {jobsByLocation.map((jobsByLocation) => (
          <JobCard
