...
function EditInternshipForm(props) {
  ...
  const [discriptionFields, setDiscriptionFields] = useState([
    { value: "", id: "" },
  ]);
  ...
  useEffect(() => {
    const fetchJobCount = async () => {
      ...
      let discriptionFieldsCount =
        oldJobData.data.getJob.jobDescription.items.length;
      let newDis = 0;
      let values = [];
  ...
  const createNewJob = async () => {
    ...
    while (iDis < lDis) {
      await API.graphql(
        graphqlOperation(updateJobDescription, {
          input: {
    ...
    while (iSkill < lSkil) {
      await API.graphql(
        graphqlOperation(updateJobSkill, {
          input: {
    ...
    while (iCri < lCri) {
      await API.graphql(
        graphqlOperation(updateJobCretaria, {
          input: {
    ...
    alert(
      "Sucessfully updated a job. Your Job will be active within 2 hrs, If not contact us."
    );
